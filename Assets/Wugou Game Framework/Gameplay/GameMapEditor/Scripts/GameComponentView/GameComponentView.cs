...
namespace Wugou.Editor.UI
{
    ...
    {
        protected GameObject componentGameObject_;
        public GameObject componentGameObject { 
            get 
            {
                return componentGameObject_;
            }
            
            set 
            {
                if (componentGameObject_ != value)
                {
                    componentGameObject_ = value;
                    if (CheckTargetValid())
                    {
                        OnNewTarget(value);
                    }
    ...
    public class GameComponentView<T> : GameComponentView where T : Component
    {
        protected T gameComponent => componentGameObject.GetComponent<T>();

        public override bool CheckTargetValid()
        {
            return componentGameObject && componentGameObject.GetComponent<T>() != null;
        }
    }
}
