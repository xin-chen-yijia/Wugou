    ...
    public class ListPropertyView : PropertyView
    {
        ...
        public override void SetValue(object target)
        {
            ...
            System.Action addOne = () =>
            {
                ...
                var viewObj = PropertyViewManager.CreatePropertyView($"element{(count - 1)}", PropertyViewManager.GetDefaultViewTypeOf(elmType), getItem.Invoke(target, new object[] { count - 1 }), (val) => { setItem.Invoke(target, new object[] { count - 1, val }); });
                if (viewObj)
                {
                    viewObj.transform.SetParent(content.transform);
                    items.Add(viewObj.gameObject);
