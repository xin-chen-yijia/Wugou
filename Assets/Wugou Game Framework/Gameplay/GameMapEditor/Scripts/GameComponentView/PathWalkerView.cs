...
namespace Wugou.Editor.UI
{
    public class PathWalkerView : GameComponentView<PathWalker>
    {
        ...
        public override void Start()
        {
            ...
            {
                float t = gameComponent.speed;
                if(float.TryParse(value, out t) )
                {
                    gameComponent.speed = t;
                }
            });

            speedInput.onDeselect.AddListener((value) =>
            {
                speedInput.text = gameComponent.speed.ToString();
            });
            ...
            addButton.onClick.AddListener(() =>
            {
                ...
                {
                    position = gameComponent.gameObject.transform.position,
                    eulerAngles = gameComponent.gameObject.transform.eulerAngles
                };
                gameComponent.points.Add(point);
                AddPointUIInternal(gameComponent.points.Count - 1);
            });
        ...
        void SetEditorActive(bool active)
        {
            ...
            if (isEditing)
            {
                cachedPosition = componentGameObject.transform.position;
                cachedRotation = componentGameObject.transform.rotation;
            }
            else
            {
                componentGameObject.transform.position = cachedPosition;
                componentGameObject.transform.rotation = cachedRotation;
            }
        ...
        private void AddPointUIInternal(int index)
        {
            ...
            // gizmos
            var gizmoObj = Instantiate<GameObject>(componentGameObject.transform.Find("Body").gameObject, entityGizmo.transform);
            gizmoObj.transform.position = point.position;
            gizmoObj.transform.eulerAngles = point.eulerAngles;
            gizmoObj.layer = GameMapEditor.mapGizmosLayer;
        ...
        public override void OnNewTarget(GameObject target)
        {
            if (target)
            {
                ...
                for(int i=0;i < gameComponent.points.Count;++i)
                {
                    AddPointUIInternal(i);
                }

                speedInput.SetTextWithoutNotify(gameComponent.speed.ToString());
            }
