...
namespace Wugou
{
    public class FlyCamera : MonoBehaviour
    {
        ...
        // 控制鼠标移动视角
        public bool freezeMouseMove = false;
        ...
        {
            LocateCameraTo(target.position, target.rotation);
        }

        public void LocateCameraTo(Vector3 position, Quaternion rotation)
        {
            if (parentTrans_ != null)
            {
                // camera view parent,so parent at camera forward
                parentTrans_.position = position + (rotation * Vector3.forward) * viewCenterDistance;

                // camera parent's origin rotation same with camera's rotation
                parentTrans_.rotation = rotation;
            }
            else
            {
