...
namespace Wugou
{
    public static class Utils
    {
        ...
        public static async Task<bool> ResizeContainer(GameObject container)
        {
            ...
            {
                Logger.Error($"{container} must active in scene when resize.");
            }
            ...
            if (container.transform.childCount > 0)
            {
                ...
                while (!firstChild && i < container.transform.childCount)
                {
                    var tmp = container.transform.GetChild(i++) as RectTransform;
                    firstChild = tmp.gameObject.activeSelf ? tmp : null;
                }
                if (!firstChild)
                {
                    return false;   // 一个都没有
                ...
                i = container.transform.childCount - 1;
                RectTransform lastChild = null;
                while (!lastChild && i >=0)
                {
                    var tmp = container.transform.GetChild(i--) as RectTransform;
                    lastChild = tmp.gameObject.activeSelf ? tmp : null;
                }
                ...
                else if (container.GetComponent<GridLayoutGroup>())   //  too complicated, make it simple
                {
                    ...
                    if (layout.constraint == GridLayoutGroup.Constraint.FixedColumnCount)
                    {
                        ...
                        containerTrans.SetSizeWithCurrentAnchors(RectTransform.Axis.Vertical, -(lastChild.anchoredPosition.y - firstChild.anchoredPosition.y) + layout.cellSize.y + layout.padding.top + layout.padding.bottom);
                    }
                    else if (layout.constraint == GridLayoutGroup.Constraint.FixedRowCount)
                    {
                        ...
                        containerTrans.SetSizeWithCurrentAnchors(RectTransform.Axis.Horizontal, lastChild.anchoredPosition.x - firstChild.anchoredPosition.x + layout.cellSize.x + layout.padding.left + layout.padding.right);
                    }
                    else
                    {
                        containerTrans.SetSizeWithCurrentAnchors(RectTransform.Axis.Vertical, -(lastChild.anchoredPosition.y - firstChild.anchoredPosition.y) + layout.cellSize.y + layout.padding.top + layout.padding.bottom);
        ...
        /// 判断鼠标是否在UI上
        /// </summary>
        /// <param name="pointerId">移动端上必须使用PointerId</param>
        /// <returns></returns>
        public static bool IsPointerOnUI(int pointerId = -1)
        {
            if(pointerId == -1)
            {
        ...
        public static bool IsPointerOnUI(Vector2 screenPosition)
        {
            ...
            eventDataCurrentPosition.position = screenPosition; // 获取画布上的 GraphicRaycaster组件
        ...
        /// <returns></returns>
        public static bool IsPointerOnUI(Canvas canvas, Vector2 screenPosition)
        {
            // 
            // 实例化点击事件
            PointerEventData eventDataCurrentPosition = new PointerEventData(EventSystem.current);  // 将点击位置的屏幕坐标赋值给点击事件
        ...
        public static bool IsTyping()
        {
            if (EventSystem.current) 
            {
                ...
                if(go && (go.GetComponent<TMPro.TMP_InputField>() || go.GetComponent<UnityEngine.UI.InputField>()))
                {
                    return true;
                }
            }
        ...
        /// <summary>
        /// 修改材质渲染模式 https://blog.csdn.net/u014635337/article/details/74127259
        /// 没用。。。。
        /// </summary>
        ...
        /// <summary>
        /// 加碰撞,可以使用linerender的mesh
        /// </summary>
        public static void AddCollider(LineRenderer lineRenderer)
        {
            ...
            //    go.transform.position = (positions[i - 1] + positions[i]) * 0.5f;
            //    go.transform.right = (positions[i] - positions[i - 1]).normalized;
            //    var collider = go.AddComponent<BoxCollider>();

            //    collider.size = new Vector3(Vector3.Distance(positions[i], positions[i - 1]), 0.1f, lineRenderer.startWidth * 0.25f);
