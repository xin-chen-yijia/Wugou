...
namespace Wugou
{
    ...
    public class QuadcopterController : MonoBehaviour
    {
        ...
        [SerializeField] private float extraForwardSpeed = 0.5f;
        ...
        public void InitSerializeProperties(string propertyParent)
        {
            var parentTrans = transform.Find(propertyParent);
            centerOfMass = parentTrans.Find("CenterOfMass");
            ForwardRightEngine = parentTrans.Find("Engine FR");
            ForwardLeftEngine = parentTrans.Find("Engine FL");
            BackRightEngine = parentTrans.Find("Engine BR");
            BackLeftEngine = parentTrans.Find("Engine BL");
        ...
        public void Move(float forward, float throttle, Direction direction = Direction.kForward)
        {
            this.throttle = throttle * (Time.deltaTime * throttleSpeed);
            this.forwardAmount = forward * (Time.deltaTime * forwardSpeed);
            switch (direction)
            {
        ...
        /// <summary>
        /// 原理：模拟四个旋翼给升力，想要前进时，通过给前面或后面的两个旋翼额外的升力，这样旋翼倾斜后，会产生一个向前或向后的力
        /// 这个方式的缺点是额外的这个升力不能给太大，太大了会翻转；如果仅仅依赖于这个升力，前进速度是没有办法加快的；
        /// 稍微改进一点：额外再增加一个推力，这样可控制前进速度；
        /// </summary>
        private void AddInternalForces()
        {
            ...
            if (newZ > 0)
            {
                ...
                rigidbody.AddForceAtPosition(transform.forward * extraForwardSpeed, centerOfMass.position, ForceMode.VelocityChange);
            }
            else if (newZ < 0)
            {
